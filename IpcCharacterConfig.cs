...
public class IpcCharacterConfig : CharacterConfig {
    ...
    public unsafe IpcCharacterConfig(Plugin plugin, IPlayerCharacter player) {
        ...
        if (Plugin.Config.ApplyStaticMinionPositions) {
            ...
            if (chr->CompanionObject != null && Utils.StaticMinions.Value.Contains(chr->CompanionObject->Character.GameObject.GetGameObjectId().ObjectId)) {
                var drawObj = chr->CompanionObject->Character.GameObject.DrawObject;
                if (drawObj != null) {
